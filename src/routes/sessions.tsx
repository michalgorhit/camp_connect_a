  ...
  const filtered = useMemo(() => {
    return sessions.filter((s) => {
      ...
      if (areaCode.trim()) {
        ...
        const hit =
          (s.postal_code ?? "").toLowerCase().startsWith(a) ||
          (s.location ?? "").toLowerCase().includes(a);
        if (!hit) return false;
      }
        ...
        <div className="mb-6 grid gap-4 rounded-2xl border border-border bg-card p-4 shadow-soft md:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-2">
            <Label className="flex items-center gap-2 text-xs uppercase tracking-wide text-muted-foreground">
              <Search className="h-3.5 w-3.5" /> Area / postal
            </Label>
            <Input
              value={areaCode}
              onChange={(e) => setAreaCode(e.target.value)}
              placeholder="e.g. Bloomington or 47401"
            />
          </div>
          <div className="space-y-2">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (active) next.delete(w.start);
                      else next.add(w.start);
...
function SessionCard({ s, canRegister }: { s: Session; canRegister: boolean }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-soft transition hover:-translate-y-0.5 hover:shadow-pop">
      <div
        className="aspect-[16/10] bg-gradient-sun"
        style={
          s.image_url
            ? {
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <ul className="mt-4 space-y-1.5 text-sm text-muted-foreground">
          <li className="flex items-center gap-2">
            <Calendar className="h-4 w-4" /> {fmt(s.start_date)} — {fmt(s.end_date)}
          </li>
          {(s.location || s.postal_code) && (
            <li className="flex items-center gap-2">
              <MapPin className="h-4 w-4" />{" "}
              {[s.location, s.postal_code].filter(Boolean).join(" · ")}
            </li>
          )}
          {(s.age_min || s.age_max) && (
            <li className="flex items-center gap-2">
              <Users className="h-4 w-4" /> Ages {s.age_min ?? "?"}–{s.age_max ?? "?"}
            </li>
          )}
          ...
          {s.registration_deadline && (
            <li>Register by {new Date(s.registration_deadline).toLocaleDateString()}</li>
          )}
        </ul>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-primary" /> Add a camp
          </DialogTitle>
          <DialogDescription>
            {step === "search"
              ? "Search first — your camp may already be listed."
              : "Add details. Capacity will be marked unknown until a vendor or admin updates it."}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === "search" ? (
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="e.g. YMCA Adventure Week"
              />
            </div>
            {title.length >= 2 && (
              <div>
                ...
                <div className="max-h-56 space-y-1 overflow-y-auto rounded-xl border border-border bg-muted/30 p-2">
                  {matches.length === 0 ? (
                    <p className="p-3 text-sm text-muted-foreground">
                      Nothing matched. You can add it as a new camp below.
                    </p>
                  ) : (
                    matches.map((s) => (
                      <Link
                        key={s.id}
                        to="/parent"
                        search={{ session: s.id } as never}
                        className="block rounded-lg p-2 text-sm hover:bg-background"
                      >
            ...
            <DialogFooter>
              ...
              <Button
                variant="hero"
                disabled={title.trim().length < 2 || !confirmed}
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <p className="rounded-lg bg-muted/40 p-2 text-xs text-muted-foreground">
              Since you're not the vendor, this camp will be tagged "Community" with capacity
              unknown. An admin or the vendor can fill in capacity later.
            </p>
            <DialogFooter>
